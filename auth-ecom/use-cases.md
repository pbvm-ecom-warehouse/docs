...
3. Cấp luôn access + refresh token (đăng nhập ngay) — **chưa verify vẫn được xem nhưng không mua được**, chỉ hiển thị nhắc xác minh
...
1. Nhập `email` → nếu tồn tại: tạo token `RESET_PASSWORD` (`customer_auth_tokens`, hạn 15p) → phát event [`customer.password_reset_requested`](../overview/data-ownership.md#các-event-đồng-bộ-giữa-wms-và-ecommerce)
